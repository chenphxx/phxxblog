...
| 族 | 视觉语言 | 动效 | 主题 |
| --- | --- | --- | --- |
| **minimal** 文档极简 | 中性灰阶, 细边框, 极淡阴影 | 仅颜色/边框过渡, 360ms | **预设 `cuanmu`(默认)** |
| **soft** 柔和抬升 | 渐变主色, 大圆角(10–16px), 多层柔和阴影 | 卡片上浮 + 左侧色条生长, 220ms | 森屿绿洲 `emerald`, 晨雾青 `mist`, 松涛墨绿 `pine`, 薄荷冰 `mint` |
| **hard** 硬朗位移 | 纯色主色, 方正小圆角(6–12px), 偏移硬阴影, 等宽标题 | 悬停位移 + 终端扫描线, 170ms | 霓虹青柠 `neon`, 春芽黄绿 `bud`, 极光青绿 `aurora` |
| **editorial** 编辑细线 | 衬线标题, 极淡阴影, 细线分割 | 标题细线浮现 + 轻微抬升, 280ms | 苔原手记 `moss`, 抹茶奶绿 `matcha`, 鼠尾草灰绿 `sage`, 青墨 `ink` |
...
| `--link` | 主色当**文字/链接**用时的颜色, 可深于主色 | 春芽黄绿链接用 `#4D7700`(5.31:1), 按钮填充仍是 `#527D00` |
